...
- A warning, ``ExtensionArrayCastingWarning``, is now emitted from base ``ExtensionArray`` methods that involve converting to object dtype. This warning is ignored by default, but elevated to ``'always'`` as part of pandas' base tests (:issue:``).
