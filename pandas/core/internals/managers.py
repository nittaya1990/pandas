...
class BlockManager(PandasObject):
    ...
    def consolidate(self) -> "BlockManager":
        ...
        bm = type(self)(self.blocks, self.axes, policy=self.policy)
...
def form_blocks(arrays, names, axes, policy):
    ...
    if len(items_dict["FloatBlock"]):
        float_blocks = _multi_blockify(items_dict["FloatBlock"], policy)
        blocks.extend(float_blocks)

    if len(items_dict["ComplexBlock"]):
        complex_blocks = _multi_blockify(items_dict["ComplexBlock"], policy)
        blocks.extend(complex_blocks)

    if len(items_dict["TimeDeltaBlock"]):
        timedelta_blocks = _multi_blockify(items_dict["TimeDeltaBlock"], policy)
        blocks.extend(timedelta_blocks)

    if len(items_dict["IntBlock"]):
        int_blocks = _multi_blockify(items_dict["IntBlock"], policy)
        blocks.extend(int_blocks)

    if len(items_dict["DatetimeBlock"]):
        datetime_blocks = _simple_blockify(items_dict["DatetimeBlock"], _NS_DTYPE, policy)
    ...
    if len(items_dict["BoolBlock"]):
        bool_blocks = _simple_blockify(items_dict["BoolBlock"], np.bool_, policy)
        blocks.extend(bool_blocks)

    if len(items_dict["ObjectBlock"]) > 0:
        object_blocks = _simple_blockify(items_dict["ObjectBlock"], np.object_, policy)
...
def _multi_blockify(tuples, dtype=None, policy=None):
    ...
    for dtype, tup_block in grouper:
        ...
        values, placement = _stack_arrays(list(tup_block), dtype, policy)
...
def _stack_arrays(tuples, dtype, policy):
    ...
    # individual blocks
    from .blocks import _block_shape

    if policy in ['split', 'column']:
        return ([_block_shape(_asarray_compat(arr), 2) for arr in arrays],
...
def _split_blocks(blocks, policy):
    ...
    if policy not in policies:
        raise ValueError("specified policy must be in [{0}]".format(
            ','.join(policies)))
    ...
    for b in blocks:
        ...
        if b.shape[0] == 1:
            ...
            for i, p in enumerate(b.mgr_locs.as_array):
                new_b = b.make_block(
                    _block_shape(b.iget(i).copy(), ndim=b.ndim),
                    placement=[p])
